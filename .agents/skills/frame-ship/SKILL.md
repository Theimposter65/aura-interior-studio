...
# /frame-ship (or /frame:ship) -- Git + PR
...
npx eslint .       # Lint
npx vitest run       # Tests (capture coverage % if the runner reports it)
npm audit      # Security (dependency/secret scan; skip row if not configured)
