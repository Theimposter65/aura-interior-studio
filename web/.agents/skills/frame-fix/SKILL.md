...
# /frame-fix (or /frame:fix) -- Parallel Review Fixes
...
- **Instructions**: apply the fix; add/adjust a targeted test only if logic changed; run `npx vitest run {file}` for the touched file; do **not** run full gates (orchestrator does that once); do **not** commit (orchestrator commits); return the final report
