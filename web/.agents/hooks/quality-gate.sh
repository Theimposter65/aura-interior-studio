...
FILE_PATH=$(node -e "try{const i=JSON.parse(process.argv[1]);process.stdout.write(i.toolCall?.args?.TargetFile||i.tool_input?.file_path||i.tool_input?.path||'')}catch{}" -- "$input" 2>/dev/null)
...
case "$FILE_PATH" in
  *.planning/*|*/.claude/*|*/.agents/*) : ;;  # not counted
  *)
...
LINT_CMD=""
LINT_FROM_CONFIG=0
if [ -f "$PROJECT_ROOT/.frame/config.json" ] && command -v node &>/dev/null; then
  CONFIG_VALID=$(node -e "try{JSON.parse(require('fs').readFileSync(process.argv[1],'utf8'));process.stdout.write('ok')}catch(e){process.stdout.write('invalid: '+e.message)}" "$PROJECT_ROOT/.frame/config.json" 2>/dev/null)
  ...
  TYPECHECK_CMD=$(node -e "try{const c=JSON.parse(require('fs').readFileSync(process.argv[1],'utf8'));process.stdout.write(c.quality?.commands?.typecheck||'')}catch{}" "$PROJECT_ROOT/.frame/config.json" 2>/dev/null)
  LINT_CMD=$(node -e "try{const c=JSON.parse(require('fs').readFileSync(process.argv[1],'utf8'));process.stdout.write(c.quality?.commands?.lint||'')}catch{}" "$PROJECT_ROOT/.frame/config.json" 2>/dev/null)
