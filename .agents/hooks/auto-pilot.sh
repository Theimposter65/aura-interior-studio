...
if [ -n "$MARKER_SESSION" ]; then
  HOOK_SESSION=$(node -e "try{const i=JSON.parse(process.argv[1]);process.stdout.write(i.session_id||i.conversationId||'')}catch{}" -- "$input" 2>/dev/null)
...
MSG="FRAME Autopilot is ON for '${FEATURE:-current feature}' and the pipeline is not finished (STATE.md: ${PHASE:-?} / ${STATUS:-?}). Continue the flight: read .planning/STATE.md and execute the next phase."

IS_ANTIGRAVITY=$(node -e "try{const i=JSON.parse(process.argv[1]);process.stdout.write((i.conversationId||i.executionNum)?'1':'')}catch{}" -- "$input" 2>/dev/null)
if [ "$IS_ANTIGRAVITY" = "1" ]; then
  node -e "process.stdout.write(JSON.stringify({decision:'continue',reason:process.argv[1]}))" -- "$MSG"
  exit 0
fi

echo "$MSG" >&2
echo "Continue the flight: read .planning/STATE.md and execute the next phase." >&2
