...
This project uses FRAME (Framework for AI-Assisted Solo Development) with Google Antigravity and Gemini models.

**Commands / Skills**:
- `/frame-init` (or `/frame:init`) — initialize project
- `/frame-daily` — morning briefing & status
...
- `/frame-audit` — unified security and performance audit
- `/frame-ship` — git + PR
- `/frame-debug <issue>` — systematic debugging
- `/frame-retrospective` — retrospective + memory update
- `/frame-evolve` — promote proven learnings into permanent rules
...
- `` — Type check
- `npx vitest run` — Test check
- `npx eslint .` — Lint check
- `npm run build` — Build check (before Ship)
...
Always respond in English. Write all specs, plans, reports, comments, and generated files in English.
