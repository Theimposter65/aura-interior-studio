...
> **Model**: gemini-2.5-pro (override via `ai.models` in `.frame/config.json`) (override via `model` in `.frame/config.json`).
...
> **Model**: gemini-2.5-flash (override via `ai.models` in `.frame/config.json`) in the panel (the panel waits on its slowest member). The opus default applies only to standalone/strict review.
