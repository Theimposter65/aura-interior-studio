...
# /frame-build (or /frame:build) -- Implementation per plan.md
...
```bash
{task's Verification: command}   # falls back to npx vitest run {test_file} if the task has none
```
