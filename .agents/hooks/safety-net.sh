...
COMMAND=$(node -e "try{const i=JSON.parse(process.argv[1]);process.stdout.write(i.toolCall?.args?.CommandLine??i.tool_input?.command??i.tool_input?.CommandLine??'')}catch{}" -- "$input" 2>/dev/null)

deny() {
  node -e "process.stdout.write(JSON.stringify({decision:'deny',reason:process.argv[1],hookSpecificOutput:{hookEventName:'PreToolUse',permissionDecision:'deny',permissionDecisionReason:process.argv[1]}}))" -- "$1"
  exit 0
}

ask() {
  node -e "process.stdout.write(JSON.stringify({decision:'ask',reason:process.argv[1],hookSpecificOutput:{hookEventName:'PreToolUse',permissionDecision:'ask',permissionDecisionReason:process.argv[1]}}))" -- "$1"
  exit 0
}
